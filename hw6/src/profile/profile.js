        ...
        jwt.verify(
            ...
            (err, payload) => {
                if (err) {
                    ...
                    //res.status(401).json({ message: `Token cannot be verified`, error: err });
                    ...
                    //res.status(401).json({ message: `JWT verify error: wrong payload` });
                }
            }
        );
    } catch (err) {
        console.error("JWT verify error: wrong token. Error: " + JSON.stringify(err));
        //res.status(401).json({ message: `Authorization was not provided` });
    }
