...
exports.createOrder = async function (req, res, next) {
    ...
    if (!req.headers['x-requestid'] || req.headers['x-requestid'] === '') {
        res.status(400).json({ message: "X-RequestId not provided" });
        return;
    }
    ...
    try {
        ...
        orderModel.createOrder(req.user.id, sum, req.body.name, req.body.productCode, prodInfo.Name,
            req.body.quantity, req.body.deliveryDate, req.body.address, req.headers['x-requestid'], (err, result) => {
                if (err) {
                    ...
                    res.status(201).json({
                        orderNumber: result.rows[0].Number,
                        message: "Order successfilly created"
                    });
                }
            });
        ...
        res.status(500).json({ message: JSON.stringify(e.message) });
    }
