...
const kafkaController = require('./authKafkaHandler');

// Создание метрик
const metricUsersTotal = new prometheus.Counter({
    name: 'my_users_total',
    ...
    userModel.getUserByEmail(req.body.email, (err, result) => {
        if (err) {
            ...
            if (result.rows && result.rows[0]) {
                ...
                userModel.getUsersCount((err, result) => {
                    if (err) {
                        next(err)
                    } else {
                        metricUsersTotal.inc({type: "basic"});
                    }
                });
